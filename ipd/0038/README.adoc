...
* As part of this we need to set clear expectations about what state is
  saved and restored across calls to `getcontxt()` and `siglongjmp()`.
...
* Updating /proc to have files for this.
...
The following are explicit non-goals of this work:
...
| Register access
| Full xsave state via ptrace | Full xsave state via ptrace | Full xsave via ptrace and /proc
...
it does cause us to pause and ask what makes the most sense.
...
entry points into libproc for FPU registers ala `Lputareg`.

XXX Flush out in more detail
...
* In general, the mcontext should assume that it has the basic ISA
  defined floating point state, but that there will be a variable sized
  extended floating point state. For example, ARM has both the NEON /
  VFPv3 and SVE extensions. We might expect the VFP/Neon bits to be part
  of the standard mcontext, while the SVE function like the xregs. If
  the save state actually overlaps (like it does with xsave), prefer
  having only a single thing rather than both.
