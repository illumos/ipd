...
address on the wire with a write bit, recive an ack, the controller
...
Under this we see our first **device**, `ltc4306@0,4a`. Let's take the
different components apart here:

* `ltc4306` is the name of the device node.
...
The `ltc4305` is a **multiplexor** (and its cousin the `ltc4306` is that
but also has a GPIO controller built in) that provides two distinct
ports.  In this case the `ltc4306` driver would be attached to this
...
instance of `i2cnex` to represet the mux itself. The unit address here
...
Controllers our guaranteed that their I/O function will only be called
from one location. Controllers indicate errors for I/O operations
through the `i2c_error_t` structure and are given routines to make it
easier to set this.
...
To facilitate I/O there are pair of request structure that the
controller use which are shared across the user I/O interfaces that are
found in
https://github.com/rmustacc/illumos-gate/blob/i2c-dev/usr/src/uts/common/sys/i2c/i2c.h[<sys/i2c/i2c.h>].
There is the `smbus_req_t` and the `i2c_req_t`:
...
* Broader support for 10-bit addresses. While there are provisions in
  ...
  pluimb it through the planned emulation devices and test suite.
