...
The plan is to hold the per-process `fi_lock` for the duration of the
snapshot, and to acquire each entry's `uf_lock` as we examine it. The
flags of each descriptor need to be inspected to honour close-on-fork,
so taking `uf_lock` per-fd is necessary anyway. This approach does not
provide a strictly atomic snapshot of the parent's descriptor table -
...
already processed (or, in the case of operations that do not take
`fi_lock`, to one we have not yet reached). The snapshot we produce is
therefore a sequence of per-fd observations rather than that of the
table at a single instant.
...
* If it has not, the externally observable result of any interleaving
  ...
  -- a state that no atomic snapshot would have produced, but one that
  could equally have arisen if thread B had completed its first
  `close(3)` before the `spawn(2)` was entered and its `close(100)`
  immediately after.
