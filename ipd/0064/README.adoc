...
BSD to `posix_spawn(3C)` in POSIX. Today, the illumos implementation
of `posix_spawn(3C)` remains a library wrapper around `vfork(2)` and
...
On illumos, both `fork(2)` and `vfork(2)` are implemented through the
`forksys` system call (`SYS_forksys`, 142). Before the child process
can be created, the kernel must stop all other LWPs in the parent
process via `holdlwps()` and wait for them to reach a safe point. For
`fork(2)`, this is necessary so that kernel stacks can be cloned. For
`vfork(2)`, the child is borrowing the parent's address space, so other
threads must not be allowed to modify it concurrently. In either case,
every thread in the parent process must be stopped before the operation
can proceed. The more threads a process has, the longer this takes.
...
`/bin/true` shows that adding threads yields almost no improvement in
spawn rate. The bottleneck is the `holdlwps()` / `continuelwps()`
cycle as each spawn must wait for every other LWP to stop and then
restart them all afterwards.
...
. Rather than copying the entire table,
  build the child's file descriptor table by examining only the
  descriptors that are needed: those referenced by file actions and
  those not marked close-on-exec. This could be more efficient for
  processes with very large descriptor tables where most descriptors are
  close-on-exec.
...
guarantees we provide. POSIX requires that the set of file descriptors
available to the spawned process be as if a `fork(2)` had occurred
followed by the specified file actions. What happens if another thread
opens or closes a file descriptor concurrently with the `spawn(2)`
call is not fully specified by POSIX.
...
* `posix_spawnp(3C)` PATH resolution. The `posix_spawnp(3C)` variant
  searches `PATH` for the executable and this is most likely better
  done in libc, although multiple calls into the new `spawn(2)`
  may be necessary; details of this need to be worked through.
* Marshalling the file actions and spawn attributes into a form
  suitable for the system call.
...
* Setting `errno` and returning the appropriate value (inc. child
  PID).
...
observation and control. The `spawn(2)` system call will need to
generate appropriate `/proc` stop events so that debuggers can
intercept the new process. The existing `PR_SYSEXIT` mechanism for
`SYS_forksys` could be used for this. A debugger that is set to stop
on fork events will also need to be able to stop on spawn events.
However, by the time the debugger can observe the child, it is already
running the new program and there is no window in which the child
exists with the parent's address space.

=== truss(1)

`truss(1)` needs to learn about the new system call so that it can
trace spawn operations and optionally follow the child process. As
with `/proc`, the main consideration is that spawned children start
life in a new program rather than as a copy of the parent.

=== mdb(1)

`mdb(1)` will likely need updates to its process target to correctly
handle spawn events. There exist separate variables to control debugger
behaviour on `fork(2)`, and on `exec(2)`; a similar control may be
required for `spawn(2)`.

=== DTrace

The existing `proc` DTrace provider includes probes for `create` (fork)
and `exec`. A `spawn(2)` call is logically both of these in a single
operation. The appropriate way to instrument this could be a new probe,
or firing both existing probes, or a combination. This will be
determined during implementation. Whatever approach is taken, it must
be possible for DTrace consumers to observe spawn events and extract
the same metadata that is available for fork and exec today. For
example, the parent and child PIDs, and the executable path.
...
* `ksh93` -- the default illumos shell uses `posix_spawn(3C)` to
...
* `popen(3C)` and `system(3C)` -- these library functions are
...
A new `spawn(2)` man page will be written documenting the system call
interface.
...
. Branded zones (such as the LX brand in some distributions) may have
  their own expectations around process creation. The interaction
  between `spawn(2)` and the brand framework needs to be considered.
