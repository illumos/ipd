...
in OmniOSCE, OpenIndiana, and SmartOS to ensure that binaries built against it
...
<<omniosce-release-schedule,OmniOSCE release schedule>>, and should cover both
the oldest supported LTS release from OmniOSCE, as well as the SmartOS LTS
...
2. Select the OmniOSCE LTS version that was supported at the time of the
   commit. In this case r151038.
3. On a system with the specified OmniOSCE version installed, build
...
* Who is responsible for initiating the annual sysroot creation process?
...
* [[omniosce-release-schedule]]https://omnios.org/schedule[OmniOSCE release schedule]
