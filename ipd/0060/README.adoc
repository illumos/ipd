...
resource types, if say a GPIO was reqiured for it or something else
...
* The `rxh_type` member will define what type of register spec this
  uses. These types are going to be considered global. Many of them will
  overlap; however, they may not all. Different platforms and
  architectures are allowed to assign other values and it is expected
  that nexi that do not understand a given type will reject it.
...
* The `rxu_nu32` indicates how many u32 values are in a single instance
  of the register specificaiton. the number will always be a minimum of
...
The following table summarizes how some of these fit together:
...
into the correspoding MEMORY and I/O types.
